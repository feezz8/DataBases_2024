...
BEGIN
    ...
    WHILE i <= (SELECT count(*) FROM episode) DO
        IF (SELECT count(*) FROM episode_selection WHERE episode_id = i) = 0 THEN 
            WHILE j <= 50 DO
                BEGIN
                    INSERT INTO episode_selection VALUES (i, FLOOR(RAND()*200+1), NULL, FLOOR(RAND()*100+1));
                END;
            SET j = j + 1;
            END WHILE;
        END IF;
...
BEGIN
    ...
    WHILE i <= (SELECT count(*) FROM episode) DO
        IF (SELECT count(*) FROM episode_judge WHERE episode_id = i) = 0 THEN
            WHILE j <= 10 DO
                BEGIN
                    INSERT INTO episode_judge VALUES (i, FLOOR(RAND()*200+1));
                END;
                SET j = j + 1;
            END WHILE;
        END IF;
...
BEGIN
    ...
    WHILE i <= (SELECT count(*) FROM episode) DO
        IF (SELECT count(*) FROM judge_rates_cook WHERE episode_id = i) = 0 THEN
            ...
            WHILE j <= 2000 DO
                BEGIN
                    INSERT INTO judge_rates_cook VALUES (i, j1,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
                    INSERT INTO judge_rates_cook VALUES (i, j2,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
                    INSERT INTO judge_rates_cook VALUES (i, j3,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
                END;
                SET j= j+1;
            END WHILE;
        END IF;
...
BEGIN
    ...
    IF (SELECT count(*) FROM episode_selection WHERE episode_id = ep_id) = 0 THEN
        WHILE j <= 100 DO
            BEGIN
                INSERT INTO episode_selection VALUES (ep_id, FLOOR(RAND()*200+1), NULL, FLOOR(RAND()*100+1));
            END;
            SET j = j + 1;
        END WHILE;
    END IF;
    ...
    IF (SELECT count(*) FROM episode_judge WHERE episode_id = ep_id) = 0 THEN
        WHILE j <= 10 DO
            BEGIN
                INSERT INTO episode_judge VALUES (ep_id, FLOOR(RAND()*200+1));
            END;
            SET j = j + 1;
        END WHILE;
    END IF;
    ...
    IF (SELECT count(*) FROM judge_rates_cook WHERE episode_id = ep_id) = 0 THEN
        ...
        WHILE j<=600 DO
            BEGIN
                INSERT INTO judge_rates_cook VALUES (ep_id, j1,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
                INSERT INTO judge_rates_cook VALUES (ep_id, j2,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
                INSERT INTO judge_rates_cook VALUES (ep_id, j3,FLOOR(RAND()*200+1), FLOOR(RAND()*5+1));
            END;
            SET j= j+1;
        END WHILE;
    END IF;
